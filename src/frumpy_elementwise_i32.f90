...
contains
  ...
  function add_i32(lhs, rhs, status) result(array)
    type(ndarray_i32), intent(in) :: lhs, rhs
    type(frumpy_status), intent(out), optional :: status
    type(ndarray_i32) :: array

    array = binary_i32(lhs, rhs, OP_ADD, status)
  end function add_i32

  function subtract_i32(lhs, rhs, status) result(array)
    type(ndarray_i32), intent(in) :: lhs, rhs
    type(frumpy_status), intent(out), optional :: status
    type(ndarray_i32) :: array

    array = binary_i32(lhs, rhs, OP_SUBTRACT, status)
  end function subtract_i32

  function multiply_i32(lhs, rhs, status) result(array)
    type(ndarray_i32), intent(in) :: lhs, rhs
    type(frumpy_status), intent(out), optional :: status
    type(ndarray_i32) :: array

    array = binary_i32(lhs, rhs, OP_MULTIPLY, status)
  end function multiply_i32

  function binary_i32(lhs, rhs, operation, status) result(array)
    type(ndarray_i32), intent(in) :: lhs, rhs
    ...
    array = owned_descriptor_i32(plan%shape, status=local_status)
    if (local_status%is_failure()) then
      if (present(status)) status = local_status
      return
    end if
    do item1 = 1_int64, array%size()
      ...
      case (OP_ADD)
        array%data(item1) = wrap_i32(int(lhs%data(lhs_position), int64) + &
          int(rhs%data(rhs_position), int64))
      case (OP_SUBTRACT)
        array%data(item1) = wrap_i32(int(lhs%data(lhs_position), int64) - &
          int(rhs%data(rhs_position), int64))
      case (OP_MULTIPLY)
  ...
  end function binary_i32

  function divide_i32(lhs, rhs, status) result(array)
    type(ndarray_i32), intent(in) :: lhs, rhs
    type(frumpy_status), intent(out), optional :: status
