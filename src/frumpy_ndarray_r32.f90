!> Concrete r32 ndarray descriptor.
module frumpy_ndarray_r32
  ...
  end type ndarray_r32

contains

  function owned_descriptor_r32(shape, order, status) result(array)
    ...
    if (resolved_order /= FRUMPY_ORDER_C .and. &
      ...
      call set_optional_status(status, FRUMPY_STATUS_UNSUPPORTED_BEHAVIOR, &
        "owned_descriptor_r32 supports only C or Fortran order")
      return
    end if
  ...
  function metadata_descriptor_r32(shape, strides, offset, status) &
      result(array)
    integer(int64), intent(in) :: shape(:)
    ...
    call assign_descriptor_metadata(array, shape, strides, offset, .false., &
      local_status)
    call set_optional_status_value(status, local_status)
  end function metadata_descriptor_r32

  function view_descriptor_r32(source, shape, strides, offset, status) &
      result(array)
    ...
    if (.not. source%has_storage()) then
      call set_optional_status(status, FRUMPY_STATUS_UNSUPPORTED_BEHAVIOR, &
        "view_descriptor_r32 requires source storage")
      return
    end if
  ...
  end function ndarray_r32_size

  function ndarray_r32_storage_size(array) result(count)
    class(ndarray_r32), intent(in) :: array
    integer(int64) :: count
  ...
  end function ndarray_r32_storage_size

  logical function ndarray_r32_has_storage(array)
    class(ndarray_r32), intent(in) :: array

    ndarray_r32_has_storage = associated(array%data)
  end function ndarray_r32_has_storage
  ...
  subroutine assign_descriptor_metadata(array, shape, strides, offset, &
    ...
    if (.not. is_valid_shape(shape)) then
      call set_status(status, FRUMPY_STATUS_INVALID_SHAPE, &
        "ndarray_r32 shape must be valid")
      return
    end if

    if (size(shape) /= size(strides)) then
      call set_status(status, FRUMPY_STATUS_INVALID_SHAPE, &
        "ndarray_r32 shape and stride ranks must match")
      return
    end if

    if (offset < 1_int64) then
      call set_status(status, FRUMPY_STATUS_INVALID_SHAPE, &
        "ndarray_r32 offset is 1-based and must be positive")
      return
    end if
    ...
    if (alloc_stat /= 0) then
      call set_status(status, FRUMPY_STATUS_ALLOCATION_FAILED, &
        "ndarray_r32 shape allocation failed")
      return
    end if
    ...
    if (alloc_stat /= 0) then
      call set_status(status, FRUMPY_STATUS_ALLOCATION_FAILED, &
        "ndarray_r32 stride allocation failed")
      return
    end if
