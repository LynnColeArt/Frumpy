!> Concrete i32 ndarray descriptor.
module frumpy_ndarray_i32
  ...
  type :: storage_i32
    ...
    integer(int32), pointer :: values(:) => null()
  end type storage_i32

  type :: ndarray_i32
    integer(int32) :: dtype_id = FRUMPY_DTYPE_I32
    ...
    integer(int32), pointer :: data(:) => null()
    type(storage_i32), pointer, private :: backing => null()
  contains
    procedure :: release => ndarray_i32_release
    procedure :: share_from => ndarray_i32_share_from
    procedure, private :: assign => ndarray_i32_assign
    generic, public :: assignment(=) => assign
    final :: ndarray_i32_finalize
    procedure :: size => ndarray_i32_size
    procedure :: storage_size => ndarray_i32_storage_size
    procedure :: has_storage => ndarray_i32_has_storage
  end type ndarray_i32

contains

  function owned_descriptor_i32(shape, order, status) result(array)
    ...
    if (resolved_order /= FRUMPY_ORDER_C .and. &
      ...
      call set_optional_status(status, FRUMPY_STATUS_UNSUPPORTED_BEHAVIOR, &
        "owned_descriptor_i32 supports only C or Fortran order")
      return
    end if
    ...
    if (alloc_stat /= 0) then
      ...
      call set_optional_status(status, FRUMPY_STATUS_ALLOCATION_FAILED, &
        "ndarray_i32 backing storage allocation failed")
      return
    end if

    array%data => array%backing%values
    call set_optional_status(status, FRUMPY_STATUS_OK)
  end function owned_descriptor_i32

  function metadata_descriptor_i32(shape, strides, offset, status) &
      result(array)
    integer(int64), intent(in) :: shape(:)
    ...
    call assign_descriptor_metadata(array, shape, strides, offset, .false., &
      local_status)
    call set_optional_status_value(status, local_status)
  end function metadata_descriptor_i32

  function view_descriptor_i32(source, shape, strides, offset, status) &
      result(array)
    type(ndarray_i32), intent(in) :: source
    integer(int64), intent(in) :: shape(:)
    integer(int64), intent(in) :: strides(:)
    integer(int64), intent(in) :: offset
    type(frumpy_status), intent(out), optional :: status
    ...
    if (.not. source%has_storage()) then
      call set_optional_status(status, FRUMPY_STATUS_UNSUPPORTED_BEHAVIOR, &
        "view_descriptor_i32 requires source storage")
      return
    end if
    ...
    call set_optional_status(status, FRUMPY_STATUS_OK)
  end function view_descriptor_i32

  !> Release this descriptor's reference; other managed aliases remain valid.
  subroutine ndarray_i32_release(array)
    class(ndarray_i32), intent(inout) :: array
    ...
    array%dtype_id = FRUMPY_DTYPE_I32
    array%rank = 0_int32
    array%offset = 1_int64
    array%owns_data = .false.
    array%is_c_contiguous = .false.
    array%is_f_contiguous = .false.
  end subroutine ndarray_i32_release

  impure elemental subroutine ndarray_i32_finalize(array)
    type(ndarray_i32), intent(inout) :: array

    call array%release()
  end subroutine ndarray_i32_finalize

  subroutine ndarray_i32_assign(destination, source)
    class(ndarray_i32), intent(inout) :: destination
    type(ndarray_i32), intent(in) :: source

    call destination%share_from(source)
  end subroutine ndarray_i32_assign

  subroutine share_descriptors_i32(destination, source, status)
    class(ndarray_i32), intent(inout) :: destination(:)
    type(ndarray_i32), intent(in) :: source(:)
    type(ndarray_i32), allocatable :: retained(:)
    type(frumpy_status), intent(out), optional :: status
    type(frumpy_status) :: local_status
    integer :: alloc_stat
    integer(int64) :: item1
    ...
    do item1 = 1_int64, size(destination, kind=int64)
      call move_descriptor_i32(destination(item1), retained(item1))
    end do
    call set_optional_status(status, FRUMPY_STATUS_OK)
  end subroutine share_descriptors_i32

  !> Copy descriptor metadata and retain storage; failure leaves destination unchanged.
  subroutine ndarray_i32_share_from(destination, source, status)
    class(ndarray_i32), intent(inout) :: destination
    type(ndarray_i32), intent(in) :: source
    type(frumpy_status), intent(out), optional :: status
    type(ndarray_i32) :: retained
    integer :: alloc_stat
    ...
    if (allocated(source%shape)) then
      ...
      if (alloc_stat /= 0) then
        call set_optional_status(status, FRUMPY_STATUS_ALLOCATION_FAILED, &
          "ndarray_i32 assignment shape allocation failed")
        return
      end if
    end if
    if (allocated(source%strides)) then
      ...
      if (alloc_stat /= 0) then
        call set_optional_status(status, FRUMPY_STATUS_ALLOCATION_FAILED, &
          "ndarray_i32 assignment strides allocation failed")
        return
      end if
    end if
    ...
    call move_descriptor_i32(destination, retained)
    call set_optional_status(status, FRUMPY_STATUS_OK)
  end subroutine ndarray_i32_share_from

  ! The source is a private staged descriptor, distinct from destination.
  subroutine move_descriptor_i32(destination, retained)
    class(ndarray_i32), intent(inout) :: destination
    type(ndarray_i32), intent(inout) :: retained
  ...
  end subroutine move_descriptor_i32

  logical function same_descriptor(lhs, rhs) result(same)
    class(ndarray_i32), intent(in) :: lhs
    type(ndarray_i32), intent(in) :: rhs
  ...
  function ndarray_i32_size(array) result(count)
    class(ndarray_i32), intent(in) :: array
    integer(int64) :: count
  ...
  end function ndarray_i32_size

  function ndarray_i32_storage_size(array) result(count)
    class(ndarray_i32), intent(in) :: array
    integer(int64) :: count
  ...
  end function ndarray_i32_storage_size

  logical function ndarray_i32_has_storage(array)
    class(ndarray_i32), intent(in) :: array

    ndarray_i32_has_storage = associated(array%data)
  end function ndarray_i32_has_storage
  ...
  subroutine assign_descriptor_metadata(array, shape, strides, offset, &
    ...
    if (.not. is_valid_shape(shape)) then
      call set_status(status, FRUMPY_STATUS_INVALID_SHAPE, &
        "ndarray_i32 shape must be valid")
      return
    end if

    if (size(shape) /= size(strides)) then
      call set_status(status, FRUMPY_STATUS_INVALID_SHAPE, &
        "ndarray_i32 shape and stride ranks must match")
      return
    end if

    if (offset < 1_int64) then
      call set_status(status, FRUMPY_STATUS_INVALID_SHAPE, &
        "ndarray_i32 offset is 1-based and must be positive")
      return
    end if
    ...
    if (alloc_stat /= 0) then
      call set_status(status, FRUMPY_STATUS_ALLOCATION_FAILED, &
        "ndarray_i32 shape allocation failed")
      return
    end if
    ...
    if (alloc_stat /= 0) then
      call set_status(status, FRUMPY_STATUS_ALLOCATION_FAILED, &
        "ndarray_i32 stride allocation failed")
      return
    end if
